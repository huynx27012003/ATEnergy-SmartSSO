...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements UserService, UserManager {
    ...
    public Result<TokenUser> getTokenUser(Long userId) {
        ...
        // Load role names for the user (if any)
        List<Long> roleIdList = userRoleService.findRoleIdListByUserId(userId);
        if (!CollectionUtils.isEmpty(roleIdList)) {
            List<openjoe.smart.sso.server.entity.Role> roles = roleService.listByIds(roleIdList);
            if (!CollectionUtils.isEmpty(roles)) {
                tokenUser.setRoles(roles.stream().map(openjoe.smart.sso.server.entity.Role::getName).collect(Collectors.toList()));
            } else {
